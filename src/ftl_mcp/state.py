...
class StateManager:
    ...
    def to_json(self, section: Optional[str] = None) -> str:
        ...
        if section == "sessions":
            return json.dumps({k: v.model_dump() for k, v in self._sessions.items()}, indent=2)
            ...
            return json.dumps({
                ...
                "inventory": self._inventory.model_dump() if self._inventory else None,
                "inventory_history": self._inventory_history,
                "current_mission": self._current_mission.model_dump() if self._current_mission else None,
